...
public final class SleepMixer {
    ...
    /// Jump the smoothed state to the targets, for a clean start.
    public func snap() {
        lock.lock(); current = targets; masterCurrent = masterTarget * fadeTarget; lock.unlock()
    }
    ...
    public func render(into out: UnsafeMutablePointer<Float>, frames: Int) {
        ...
        scratch.withUnsafeMutableBufferPointer { buf in
            ...
            for (index, voice) in voices.enumerated() {
                ...
                let start = g
                g += (target - g) * (1 - pow(1 - smoothing, Float(frames)))
                current[index] = g
        ...
        masterCurrent += (master - masterCurrent) * (1 - pow(1 - smoothing, Float(frames)))
